...
export default {
  ...
  computed: {
    ...
    currentMonthRecords() {
      return this.allData.filter((record) => record.date == this.selectedMonth);
    },
    currentMonthEarnings() {
      const mapTestToCost = (testID) => {
        const matchingTests = this.tests.filter((test) => test._id == testID);
        if (matchingTests.length) {
          return matchingTests[0].cost;
        } else {
          // this.containsDeletedTest(testID);
          return 0;
        }
      };
    ...
    // currentMonthAllTests() {
    //   const testList = this.currentMonthRecords.map((record) => record.tests).flat();
    //   const counts;
