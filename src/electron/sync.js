    ...
    try {
        ...
        if (e.reason == "missing" || e.reason == "deleted") {
            ...
            try {
                ...
                log.error(
                    "Sync.js: queueRecordSync() -> can't get sync queue",
                    error
                );
            }
        } else {
    ...
    try {
        ...
        if (e.reason == "missing" || e.reason == "deleted") {
            ...
            try {
                ...
                log.error(
                    "Sync.js: getSyncQueue() -> can't get sync queue",
                    error
                );
            }
        } else {
...
export const removeItem = async (id) => {
    try {
        ...
        queueDoc.queue = queueDoc.queue.filter(
...
export const syncWithCloudData = async (data) => {
    ...
    stagedDelta.added.forEach(
        async (doc) => await db.addCloudStaged(doc, true)
    );
    stagedDelta.changed.forEach(
        async (doc) => await db.updateStaged(doc, true)
    );
    stagedDelta.deleted.forEach(
        async (doc) => await db.removeStaged(doc, null, true)
    );

    recordsDelta.added.forEach(
        async (doc) => await db.addCloudRecord(doc, true)
    );
    recordsDelta.changed.forEach(
        async (doc) => await db.updateRecord(doc, true)
    );
    ...
    testsDelta.deleted.forEach(
        async (doc) => await db.removeTest(doc._id, true)
    );
