        ...
        if (filter) {
            return allRecords
                ...
                .filter((record) => {
                    return record.date
                        .toLowerCase()
                        .includes(filter.dateFilter);
                })
                .filter((record) => {
                    return record.age.toLowerCase().includes(filter.ageFilter);
                })
                .filter((record) => {
                    return record.specimen
                        .toLowerCase()
                        .includes(filter.specimenFilter);
                })
                .filter((record) => {
                    return record.referer
                        .toLowerCase()
                        .includes(filter.refererFilter);
                });
        } else {
            return allRecords;
        }
    ...
    if (query.rows && query.rows.length) {
        ...
        if (filter) {
            return allRecords
                ...
                .filter((record) => {
                    return record.aspNote
                        .toLowerCase()
                        .includes(filter.aspNoteFilter);
                })
                .filter((record) => {
...
export const addRecord = async (record, skip_queue) => {
    ...
    if (!skip_queue) {
        try {
            ...
            log.error(
                "db.js: addRecord failed - error adding to sync queue",
                e
            );
        }
    }
...
export const addTemplate = async (organ, template, skip_queue) => {
    ...
    template._id = Math.random().toString(36).substr(2, 9);
    currentOrgan.templates.push(template);
    ...
    if (!skip_queue) {
        try {
            ...
            log.error(
                "db.js: addTemplate failed - error adding to sync queue",
                e
            );
        }
    }
...
export const updateStaged = async (record, skip_queue) => {
    const getRev = async (recordID) => {
        try {
            ...
            log.error(
                "db.js: updateStaged failed - couldn't get old record",
                error
            );
            return;
        }
    ...
    if (!skip_queue) {
        try {
            ...
            log.error(
                "db.js: updateStaged failed - error adding to sync queue",
                e
            );
        }
    }
...
export const updateRecord = async (record, skip_queue) => {
    ...
    if (!skip_queue) {
        try {
            ...
            log.error(
                "db.js: updateRecord failed - error adding to sync queue",
                e
            );
        }
    }
...
export const updateTest = async (test, skip_queue) => {
    ...
    if (!skip_queue) {
        try {
            ...
            log.error(
                "db.js: updateTest failed - error adding to sync queue",
                e
            );
        }
    }
...
export const updateTemplate = async (organ, template) => {
    ...
    try {
        ...
        log.error(
            "db.js: updateTemplate failed - error adding to sync queue",
            e
        );
    }
};
...
export const removeStaged = async (id_doc, rev, skip_queue) => {
    ...
    if (rev) {
        try {
            ...
            log.error(
                "db.js: removeStaged failed - error getting staged record",
                e
            );
        }
...
export const removeTest = async (id, skip_queue) => {
    try {
        ...
        log.error(
            "db.js: removeTest failed - error adding to sync queue",
            error
        );
    }
};
...
export const removeTemplate = async (organ, templateID, skip_queue) => {
    ...
    if (!skip_queue) {
        try {
            ...
            log.error(
                "db.js: removeTemplate failed - error adding to sync queue",
                e
            );
        }
    }
