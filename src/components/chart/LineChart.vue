...
export interface LineChartProps {
    dataGroups: Array<Array<DataPoint>>;
}
...
let d3El: d3.Selection<SVGElement, unknown, null, undefined>;
let height = 0,
    width = 0;

const draw = () => {
    const dataGroups = props.dataGroups;
    ...
    const xScale = d3.scaleLinear().domain([0, maxXVal]).range([0, width]);
    const yScale = d3.scaleLinear().domain([0, maxYVal]).range([height, 0]);
    const line = d3
        .line()
        .x((d: any) => xScale(d.x))
        .y((d: any) => yScale(d.y))
    ...
    d3El.append("g").call(xAxis).attr("transform", `translate(0, ${height})`);
...
onMounted(() => {
    ...
    //@ts-ignore
    d3El = d3.select(svg.value as SVGElement);
...
function onResize() {
    ...
    height = svg.value.parentElement?.clientHeight || 0;
    width = svg.value.parentElement?.clientWidth || 0;
